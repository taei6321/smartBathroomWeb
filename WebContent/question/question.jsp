	...
	<div class="container">
		<div class="row">
				...
				<tbody>
					...
					%>
							<tr>
								...
								<td><a href="questionView.jsp?questionID=<%= list.get(i).getQuestionID()%>"><%= list.get(i).getQuestionTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></a></td>
								<td><%= list.get(i).getUserID() %></td>
								<td><%= list.get(i).getQuestionDate().substring(0, 11) + list.get(i).getQuestionDate().substring(11, 13) + "시 " +  list.get(i).getQuestionDate().substring(14, 16) + "분 "%></td>
							</tr>
					<%
						}
					%>
			...
			<%
				if(pageNumber != 1){
			%>
					<a href="question.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arraw-left">이전</a>
			<%
				}
				if(questionDAO.nextPage(pageNumber + 1)){
			%>
					<a href="question.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arraw-left">다음</a>
			<%
				}
				
			%>
					<a href="questionWrite.jsp" class="btn btn-primary pull-right">글쓰기</a>
