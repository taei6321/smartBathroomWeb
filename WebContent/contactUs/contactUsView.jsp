...
<body>
	...
	<%
		...
		if(contactUsID == 0){
			...
			script.println("location.href = 'contactUs.jsp'"); // 사용자를 이전페이지로 돌려 보내는 것
			script.println("</script>");
		}
	...
	<div class="container">
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<thead>
					<tr>
						<th colspan="3" style="background-color: #eeeeee; text-align: center;">고객 문의글 보기</th>
					</tr>
				</thead>
				...
				<tbody>
					<tr>
						...
						<td colspan="2"><%= contactUs.getContactUsTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
					</tr>
					<tr>
						<td >작성자</td>
						<td colspan="2"><%= contactUs.getUserID() %></td>
					</tr>
					<tr>
						<td >작성일자</td>
						<td colspan="2"><%= contactUs.getContactUsDate().substring(0, 11) + contactUs.getContactUsDate().substring(11, 13) + "시 " + contactUs.getContactUsDate().substring(14, 16) + "분 "%></td>
					
					</tr>
					<tr>
						<td >내용</td>
						<td colspan="2" style="min-height: 200px; text-align: left;"><%= contactUs.getContactUsContent().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
					</tr>
				</tbody>

			</table>
			<a href="contactUs.jsp" class="btn btn-primary">목록</a>
			<%
				if(userID != null && userID.equals(contactUs.getUserID())){
			%>
					<a href="contactUsUpdate.jsp?contactUsID=<%=contactUsID %>" class="btn btn-primary">수정</a>
					<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="contactUsDeleteAction.jsp?contactUsID=<%=contactUsID %>" class="btn btn-primary">삭제</a>
			<%
				}
			%>
